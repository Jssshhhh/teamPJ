...
driver.find_element(By.XPATH, '//*[@id="boxList"]/ul/li[8]/a').click()  # 롯데 클릭
...
def scrape_month(year, month):
    ...
    for idx, row in enumerate(rows):
        try:
            ...
            years.append(year)  # 현재 연도를 추가

            print(current_day, ballpark, farteam, farteam_point, hometeam_point, hometeam, year)  # 연도 출력 추가
...
while True:
    ...
    if month > 10:
        ...
        year += 1  # 연도 증가
        print(f"Moving to year: {year}")  # 연도 변경 시 출력
...
df['구장'] = df['구장'].replace('울산', '사직')

# CSV 파일로 저장
df.to_csv("롯데_데이터3.csv", index=False, encoding='utf-8-sig')
print(df)
driver.quit()