...
home_location = (35.19403166, 129.06151836)

# 두산 베어스의 각 원정 경기 승률을 지도에 표시
for team, data in lotte_away_win_rates.items():
    color = get_color_from_winrate(data['away_win_rate'])
    # 선의 굵기를 승률에 비례하여 조정
    line_width = 5 + (data['away_win_rate'] - 0.4) * 10  # 굵기 조정, 예: 0.4일 때 0, 0.6일 때 2
    line_width = min(15, line_width)  # 최소 굵기 1로 설정
    
    folium.PolyLine(
        locations=[home_location, data['location']],  # 두산 베어스 홈구장과 원정 구장을 연결
        color=color,
        weight=line_width,  # 승률에 따른 선의 굵기
        opacity=0.7
    ).add_to(m)
...
# 지도를 HTML로 저장
m.save("./data/lotte_away_win_rates_line.html")
